...
| --- | --- | --- | --- | --- |
| [#9 — Colima gVisor Go SDK RC acceptance report](https://github.com/geminixiang/agent-sandbox-platform/issues/9) | External standard-library-only Go module `v0.2.0-rc.1`, one-node arm64 Colima+k3s, gVisor coding/browser Pools | Pass; no Claims remaining | Rotating tokens, lifecycle, checked commands, text/bytes, 10 MiB streaming, discovery/reconnect, Chromium, module isolation, and cleanup | — |
| [#8 — Colima gVisor TypeScript SDK RC acceptance report](https://github.com/geminixiang/agent-sandbox-platform/issues/8) | Clean-installed `@geminixiang/sandbox-sdk@0.2.0-rc.1`, Node 24, one-node arm64 Colima+k3s, gVisor coding/browser Pools | Pass; 31.6 seconds, no Claims remaining | Rotating short-lived tokens, lifecycle, typed command failures, immutable models, text/bytes, 10 MiB streaming, discovery/reconnect, two-Subject isolation, Chromium, package provenance, and cleanup | — |
| [#7 — Colima gVisor durable command supervisor gate report](https://github.com/geminixiang/agent-sandbox-platform/issues/7) | Isolated root-supervisor prototype on one-node arm64 Colima+k3s+gVisor | Blocked; core checks passed, production containment unavailable | Identity/capabilities, peer auth, idempotency, replay, spool bounds, stdin, signals, restart recovery, `setsid` escape, cgroup availability, and cleanup | [ADR 0005](adr/0005-durable-command-runtime-prototype.md) |
| [#6 — Colima gVisor lease discovery and reconnect report](https://github.com/geminixiang/agent-sandbox-platform/issues/6) | One-node arm64 Colima+k3s, Agent Sandbox v0.5.2, gVisor coding Pool | Pass; three tenant-scoped Leases cleaned up | Pool-filtered pagination, encrypted cursor isolation, control-plane restart, reconnect exec, Workspace persistence, and cleanup | — |
